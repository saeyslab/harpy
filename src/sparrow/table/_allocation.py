...
def bin(
    ...
    sdata
        The SpatialData object.
    adata
        An AnnData object containing unbinned gene counts. The index (`adata.obs.index`) will be matched with the column specified by `name_barcode_id` in the `points_layer`.
    points_layer
        The points layer in `sdata` that contains the spatial locations of the barcodes.
    labels_layer
        The labels layer (e.g., segmentation mask, or a grid generated by `sp.im.add_grid_labels_layer`) in `sdata` used to bin barcodes into cells or regions.
    output_layer
        The table layer in `sdata` in which to save the AnnData object with the binned counts per cell or region.
    to_coordinate_system
        The coordinate system that holds `labels_layer` and `points_layer`.
    name_x
        The column name in the `points_layer` representing the x-coordinate of the barcodes.
    name_y
        The column name in the `points_layer` representing the y-coordinate of the barcodes.
    name_z
        The column name in the `points_layer` representing the z-coordinate (if applicable) of the barcode.
    name_barcode_id
        The column name in the `points_layer` holding unique identifiers for the barcodes. Will be matched with `adata.obs.index`.
    chunks
        Chunk sizes for processing. Can be a string, integer, or tuple of integers.
        Consider setting the chunks to a relatively high value to speed up processing,
        taking into account the available memory of your system.
    append
        If set to True, and the `labels_layer` does not yet exist as a `_REGION_KEY` in `sdata.tables[output_layer].obs`,
        the binned counts obtained during the current function call will be appended (along axis=0) to any existing count values
        within the SpatialData object's table attribute. If False, and `overwrite` is set to True, any existing data in `sdata.tables[output_layer]` will be overwritten by the newly binned counts.
    overwrite
        If True, overwrites the `output_layer` if it already exists in `sdata`.
    ...
    combined_partitions = _aggregate(
        se=se,  #  TODO if not given, generate one, based on barcodes_location, using buffer, and then aggregate to this generated labels layer.
        ddf=sdata.points[points_layer],
        chunks=chunks,
        to_coordinate_system=to_coordinate_system,
        name_x=name_x,
    ...
    if name_z in combined_partitions:
        coordinates = combined_partitions.groupby(_CELL_INDEX)[name_x, name_y, name_z].mean()
    else:
    ...
    assert (
        _mask.all()
    ), f"Some spots of points layer '{points_layer}', where assigned to more than one bin in '{labels_layer}'."
    ...
    merged = pd.merge(adata.obs, cell_counts[_CELL_INDEX], left_index=True, right_index=True, how="inner")
    assert merged.shape[0] != 0, (
        "Result after merging AnnData object, passed via 'adata' parameter, "
        "with aggregated spots is empty. "
        f"Please check that index of 'adata.obs' corresponds to '{name_barcode_id}' of points layer '{points_layer}'."
    )
    adata = adata[merged.index]
    adata.obs = merged

    group_labels = adata.obs[_CELL_INDEX].values
    unique_labels, group_indices = np.unique(group_labels, return_inverse=True)
    N_groups = len(unique_labels)

    assert issparse(adata.X), "Currently only AnnData objects with a sparse feature matrix are supported."
    ...
    adata = AnnData(
        X=summed_counts, obs=pd.DataFrame(unique_labels, columns=[_INSTANCE_KEY], index=unique_labels), var=adata.var
    )
    ...
    adata.obsm["spatial"] = coordinates[coordinates.index.isin(adata.obs[_INSTANCE_KEY])].sort_index().values
