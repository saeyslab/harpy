...
def transcript_density(
    ...
    # get image boundary from last image layer if image_name is None
    ...
    if crd is None:
        ...
        if _crd != crd:
            log.warning(
                f"Provided crd didn't fully fit within the image layer '{image_name}' with image boundary '{img_boundary}'. "
