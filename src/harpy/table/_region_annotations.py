...
def compute_distance_to_shapes(
    ...
    """
    Compute distances from cells (using their centroid coordinates) to polygons in a shapes layer and update the sdata table layer.
    ...
    if has_polygons:
        ...
        if "nearest_inner_edge_grouped" in modes:
            log.info("Calculating 'nearest_inner_edge_grouped' distances'")
            for name, group in gdf_polygons.groupby(shape_names_column):
                hole_lines = []
                hole_labels = []
        ...
        if "all_inner_edges" in modes and not holes_gdf.empty:
            log.info("Calculating 'all_inner_edges' distances'")
