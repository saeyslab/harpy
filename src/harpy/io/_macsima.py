...
def macsima(
    ...
    include_roi_id_in_channel_name: bool = True,
    ...
    **cycle_scantype_channelname_reagent** if `include_roi_id_in_channel_name=False`).
    ...
    path
        ...
        The function searches recursively for preprocessed `.tif` files under each path
        (subdirectories allowed) as generated by MACS IQ VIEW.
        All files discovered for a single path are expected to belong to the same ROI.
    c_subset
    ...
    include_roi_id_in_channel_name
        If set to `False` will omit the ROI ID from channel names.
    imread_kwargs
    ...
    for _path in path:
        image_name, se = _macsima(
            ...
            include_roi_id_in_channel_name=include_roi_id_in_channel_name,
...
def _macsima(
    ...
    if "chunks" in image_models_kwargs.keys():
        ...
        chunks = None
    path_list = glob.glob(f"{path}/**/*{MacsimaKeys.IMAGE_OMETIF}", recursive=True)
    if not path_list:
    ...
    if include_roi_id_in_channel_name:
